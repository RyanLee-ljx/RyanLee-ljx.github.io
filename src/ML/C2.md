...
**Definition**: The state value represents the expected return (total accumulated rewards) starting from a specific state 
$s$, following a certain policy $\pi$. It is the long-term reward the agent expects to receive starting from state $s$ and following policy 
$\pi$.
...
*Definition*: The action value represents the expected return starting from a specific state $s$, taking an action $a$, and then following a certain policy $\pi$.
...
Given a deterministic policy, all actions that the policy not select at a certain state have a zero action value. Is this correct?

The answer is no.

First, even if an action would not be selected by a policy, it still has an action value. In the above example, although policy $\pi$ does not 
take $a_1$ at $s_1$, we can still calculate its action value by observing what we would obtain after taking this action.

Second, why do we care about the actions that the given policy would not select? Although some actions cannot be possibly selected by a given policy, this does not mean that these actions are not good. It is possible that the given policy is not good, so it cannot select the best action. The purpose of reinforcement learning is to find optimal policies. To that end, we must keep exploring all actions to determine better
