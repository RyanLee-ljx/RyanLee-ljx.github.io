...
::: tip **为什么常见的损失函数MSE和交叉熵等同于上述的MLE的表达？**
...
由于$\theta$可能有许多不同的取值，对应每种取值都有可能发生当前观测数据$x$的情况，那么对应所有不同$\theta$下，发生观测$x$的情况可以表示为：$p(x) = \int P(x | \theta) P(\theta) d\theta$ (或 $p(x) = \int P(x, \theta)d\theta$)。
...
MAP只是一种“点估计”，它只找到了后验分布 $P(\theta | x)$ 中概率最大的那个点 $\theta_{MAP}$。这在实践中很有用（例如L2正则化），但它并没有得到完整的后验分布。

为什么我们如此需要这个完整的分布呢？因为它可以用来进行贝叶斯预测 (Bayesian Prediction)。

假设我们有了一个新的、未见过的数据点 $\tilde{x}$，我们想预测它的概率。我们不是用某一个 $\theta_{MAP}$ 来进行预测 $P(\tilde{x} | \theta_{MAP})$，而是利用整个后验分布，计算一个加权平均。这个权重就是 $\theta$ 的后验概率 $P(\theta | x)$。

这就是后验预测分布 (Posterior Predictive Distribution)，其公式如下：

$$
P(\tilde{x} | x) = \int P(\tilde{x} | \theta) P(\theta | x) d\theta
$$

其含义是：我们考虑了所有可能的参数 $\theta$，并根据我们观察到的数据 $x$ 来判断它们各自的可能性 $P(\theta | x)$，然后将它们对 $\tilde{x}$ 的预测 $P(\tilde{x} | \theta)$ 进行积分，本质就是加权求和。

这样做能完美地量化不确定性（考虑所有可能的情况）。


好，现在我们已经明白了机器学习中两大学派的基本观点，尤其是贝叶斯学派的基本思想，下一节，我们将要介绍隐变量及变分推断，进而引出VAE和Diffsuion model。