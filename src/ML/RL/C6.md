...
RM is a model-free approach without knowing $g$. This is important since $g$ can be much more complex in some nueral networks. In this circumstance, we only know the values —— the input to the model and the noisy observation of the model's output. RM utilizes these two values to find the root of the model.
...
![An example for illustrating the convergence of the RM algorithm](https://github.com/RyanLee-ljx/RyanLee-ljx.github.io/blob/image/RL/C6/11.png?raw=true)
